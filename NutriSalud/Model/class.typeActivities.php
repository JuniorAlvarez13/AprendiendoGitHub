...
static function consultar($token) {
    ...
    $URL = "https://nutricontrol.co/api/typeActivities";
...
static function POST($nombre,$token){
...
static function consultaId($id,$token){
    ...
    $URL = "https://nutricontrol.co/api/typeActivities".$id;
    $headers 	= array('Authorization: Bearer ' . $token);
    $ch 		= curl_init();
    curl_setopt($ch,CURLOPT_URL,$URL);
    curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
...
static function actualizar($nombre,$id,$token){
